...
wire busy;
reg [15:0] CPU_A_idx, CPU_B_idx, CPU_C_idx;
reg [15:0] TPU_A_idx, TPU_B_idx, TPU_C_idx;      
reg [15:0] READ_A_idx, READ_B_idx, READ_C_idx;
reg [15:0] M_reg, N_reg, K_reg;                  
reg in_valid_reg;
reg [31:0] inputoffset_reg;

wire tpu_A_wr_en, tpu_B_wr_en, tpu_C_wr_en;
wire [15:0] tpu_A_index, tpu_B_index, tpu_C_index;
...
wire [15:0] num_groups_A = (M_reg + 16'd3) >> 2;
wire [15:0] num_groups_B = (N_reg + 16'd3) >> 2;

wire [31:0] total_words_A = num_groups_A * K_reg;
wire [31:0] total_words_B = num_groups_B * K_reg;


wire A_idx_out_of_bound = (TPU_A_idx >= total_words_A) ? 1'b1 : 1'b0;
wire B_idx_out_of_bound = (TPU_B_idx >= total_words_B) ? 1'b1 : 1'b0;
...
always @(*) begin
    if(busy || (State == IDLE && cmd_payload_function_id[9:3] == 10)) begin
        TPU_A_idx = tpu_A_index[15:0];
        TPU_B_idx = tpu_B_index[15:0];
        TPU_C_idx = tpu_C_index[15:0];
        C_wr_en = tpu_C_wr_en;
    end
    else begin
        ...
        case(State)
            IDLE: begin
                if (cmd_valid) begin
                    if (cmd_payload_function_id[9:3] == 0) begin
                        A_wr_en <= 1'b1;
                        CPU_A_idx <= cmd_payload_inputs_1[15:0];
                        A_data_in <= cmd_payload_inputs_0;
                    end
                    else if (cmd_payload_function_id[9:3] == 1) begin
                        B_wr_en <= 1'b1;
                        CPU_B_idx <= cmd_payload_inputs_1[15:0];
                        B_data_in <= cmd_payload_inputs_0;
                    end
                    else if (cmd_payload_function_id[9:3] == 2) begin
                        K_reg <= cmd_payload_inputs_0[15:0];
                        M_reg <= cmd_payload_inputs_1[31:16];
                        N_reg <= cmd_payload_inputs_1[15:0];
                    end
                    else if (cmd_payload_function_id[9:3] == 3) begin
                        READ_A_idx <= cmd_payload_inputs_1[15:0];
                    end
                    else if (cmd_payload_function_id[9:3] == 4) begin
                        READ_B_idx <= cmd_payload_inputs_1[15:0];
                    end
                        ...
                        READ_C_idx <= cmd_payload_inputs_1[15:0];
                    end
                    else if (cmd_payload_function_id[9:3] == 11) begin
