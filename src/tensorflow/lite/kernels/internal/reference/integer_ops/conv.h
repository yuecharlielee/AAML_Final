...
// Loop unrolling parameter - adjust this to change unroll factor
// Common values: 4, 8, 16, 32, 64
// Larger values = better performance but bigger code size
#ifndef CONV_LOOP_UNROLL_FACTOR
#define CONV_LOOP_UNROLL_FACTOR 8
#endif
            ...
            for (int n_base = 0; n_base < N; n_base += TILE_SIZE) {
                ...
                for (int j = 0; j < tile_width; ++j) {
                    ...
                    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                        ...
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            ...
                            bool is_point_inside_image = (in_x >= 0) && (in_x < input_width) && (in_y >= 0) && (in_y < input_height);

                            // Unroll in_channel loop for optimal performance
                            // Hoist branch outside unrolled loop to reduce branch penalty
                            int in_channel = 0;
                            const int UNROLL = CONV_LOOP_UNROLL_FACTOR;
                            int depth_limit = (filter_input_depth / UNROLL) * UNROLL;
                            ...
                            if (is_point_inside_image) {
                                ...
                                for (; in_channel < filter_input_depth; ++in_channel) {
                                    im2col_buf[j][k] = input_data[Offset(input_shape, batch, in_y, in_x, in_channel + group * filter_input_depth)];
                                    cfu_op0(0, im2col_buf[j][k], (j << 16) | k);
                                    k++;
                                }
                            } else {
                                // Padding path: use pre-initialized values (-input_offset)
                                for (; in_channel < depth_limit; in_channel += UNROLL) {
                                    for (int u = 0; u < UNROLL; ++u) {
                ...
                for (int m_base = 0; m_base < M; m_base += TILE_SIZE) {
                    ...
                    for (int i = 0; i < tile_height; ++i) {
                        ...
                        for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                                // Unroll in_channel for optimal performance
                                int in_channel = 0;
                                const int UNROLL = CONV_LOOP_UNROLL_FACTOR;
                                int depth_limit = (filter_input_depth / UNROLL) * UNROLL;
                                
                                for (; in_channel < depth_limit; in_channel += UNROLL) {
                                    int filter_offset = Offset(filter_shape, out_channel, filter_y, filter_x, in_channel);
                                    
                                    // Load filter values
                                    int32_t filter_vals[CONV_LOOP_UNROLL_FACTOR];
                                    for (int u = 0; u < UNROLL; ++u) {
                                        filter_vals[u] = filter_data[filter_offset + u];
                                    }
                                    
                                    // Process filter values
                                    for (int u = 0; u < UNROLL; ++u) {
                                        if (filter_vals[u] != 0) {
                                          cfu_op0(3, filter_vals[u], (i << 16) | (k+u));
                                        }
                                    }
                    ...
                    // Unroll result readback for optimal throughput
                    for (int i = 0; i < tile_height; ++i) {
                        int j = 0;
                        const int UNROLL = CONV_LOOP_UNROLL_FACTOR;
                        int width_limit = (tile_width / UNROLL) * UNROLL;
                        
                        for (; j < width_limit; j += UNROLL) {
                            // Read results together
                            int32_t results[CONV_LOOP_UNROLL_FACTOR];
                            for (int u = 0; u < UNROLL; ++u) {
                                results[u] = static_cast<int32_t>(cfu_op0(4, i, j+u));
                            }
                            
                            // Process results
                            for (int u = 0; u < UNROLL; ++u) {
                                int m_curr = m_base + i;
                                int n_curr = n_base + j + u;
                                int out_y = n_curr / output_width;
                                int out_x = n_curr % output_width;
                                
                                tile_acc[i][j+u] = results[u];
                                
                                if (bias_data) {
                                    tile_acc[i][j+u] += bias_data[m_curr];
                                }
                                tile_acc[i][j+u] = MultiplyByQuantizedMultiplier(tile_acc[i][j+u], output_multiplier[m_curr], output_shift[m_curr]);
                                tile_acc[i][j+u] += output_offset;
                                tile_acc[i][j+u] = std::max(tile_acc[i][j+u], output_activation_min);
                                tile_acc[i][j+u] = std::min(tile_acc[i][j+u], output_activation_max);
                                output_data[Offset(output_shape, batch, out_y, out_x, m_curr)] = static_cast<int8_t>(tile_acc[i][j+u]);
                            }
                        }
                        ...
                        for (; j < tile_width; ++j) {
                            ...
                            tile_acc[i][j] = MultiplyByQuantizedMultiplier(tile_acc[i][j], output_multiplier[out_channel], output_shift[out_channel]);
                            ...
                            output_data[Offset(output_shape, batch, out_y, out_x, out_channel)] = static_cast<int8_t>(tile_acc[i][j]);
                        }
                    }
            ...
            for (int out_y = 0; out_y < output_height; ++out_y) {
                ...
                for (int out_x = 0; out_x < output_width; ++out_x) {
                    ...
                    for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
                        ...
                        int32_t scaled_acc = MultiplyByQuantizedMultiplier(acc, output_multiplier[out_channel], output_shift[out_channel]);
